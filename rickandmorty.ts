...
const characterResponse: {info: {}, results: Character[]} = {
...
const getAliveHumans: (allCharacters: Character[]) => Character[] = (allCharacters) => {
    return allCharacters.filter((currentElement) => currentElement.status === "Alive" && currentElement.species === "Human")
}

const characterNames: (allCharacters: Character[]) => string[] = (allCharacters) => {
    return allCharacters.map((currentElement) => currentElement.name)
}

const namesAndOrigin: (allCharacters: Character[]) => {name: string, origin: string}[] = (allCharacters) => {
    return allCharacters.map((currentElement) => {
        return {name: currentElement.name, origin: currentElement.origin.name}
    })
}

//console.log(aliveHumans(characterResponse.results))
//console.log(characterNames(characterResponse.results))
//console.log(namesAndOrigin(characterResponse.results))
